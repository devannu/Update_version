...
// ---------------- ENTRY / EXIT UPDATE ----------------
function updateVisitorFromQR(scanned) {

    let data = JSON.parse(localStorage.getItem("visitors")) || [];

    // Find existing visitor (same ID)
    let index = data.findIndex(v => v.id == scanned.id);

    // ----------  FIRST SCAN = IN ----------
    if (index !== -1 && data[index].status === "Pending") {

        data[index].timeIn = new Date().toLocaleTimeString();
        data[index].status = "In";

        alert("Entry Marked!\nVisitor: " + data[index].name);
    }

    // ---------- SECOND SCAN = OUT ----------
    else if (index !== -1 && data[index].status === "In" && data[index].timeOut === "") {
    ...
    // ---------- THIRD SCAN = BLOCK ----------
    else if (index !== -1 && data[index].status === "Out") {
        alert("⚠ Visitor already exited!\nFurther scanning blocked.");
        return;
    }

    // ---------- Visitor NOT FOUND ----------
    else if (index === -1) {
        alert("Visitor Record Not Found!\nQR is invalid or deleted.");
        return;
    }
